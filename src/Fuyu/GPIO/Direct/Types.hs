...
data LineValue 
  = LineActive   -- ^ Line is logically active (1 as C int).
...
data LineDirection 
  = DirAsIs   -- ^ Request the line(s), but don't change direction (1 as C int).  
...
data LineEdge 
  = EdgeNone    -- ^ Line edge detection is disabled (1 as C int). 
...
data LineBias
  = BiasAsIs     -- ^ Don't change the bias setting when applying line config (1 as C int). 
...
data LineDrive 
  = PushPull   -- ^ Drive setting is push-pull (1 as C int). 
...
data LineClock 
  = Monotonic -- ^ Line uses the monotonic clock for edge event timestamps (1 as C int).
...
data EdgeEventType 
  = Rising  -- ^ Event triggered on a rising edge (1 as C int).
